...
#GRID
tmap <- expand.grid(lam1 = seq(121^2, 130^2, length = 10)^(1/2),
                    lam2 = seq(35^2, 42^2, length = 10)^(1/2),
                    lam3 = seq(10, 25, length = 10))
...
#result directory
respath <- "/home/cconley/scratch-data/neta-metabric/disc-cv-vote/lumb/05/"
library(spacemap)
tictoc <- system.time({cvsmap <- cvVote(Y = Y, X = X,
                                        ...
                                        resPath = respath,
                                        tol = 1e-4, cdmax = 40e7)})
save.image(file = file.path(respath, "lumb-05.rda"))
stopCluster(cl)
