...
prev <- new.env()
tunedir <- "~/scratch-data/neta-metabric/disc-cv-vote/her2/05"
load(file = file.path(tunedir, "her2-05.rda"), envir = prev)
...
#result directory
respath <- "/home/cconley/scratch-data/neta-metabric/disc-boot-vote/her2/01/"
...
tictoc <- system.time({ens <- spacemap::bootEnsemble(Y = Y, X = X, tune = tune,
						     ...
						     resPath = respath,
                                                     seed = seed, p0 = 1.0,
                                                     tol = 1e-4, cdmax = 90e7)})
save.image(file = file.path(respath, "her2-boot-vote-01.rda"))
...
#  SAVE RESULTS  #
##################
saveRDS(object = bv, file = file.path(respath, "her2-boot-vote-01.rds"))
save.image(file = file.path(respath, "her2-boot-vote-01.rda"))
