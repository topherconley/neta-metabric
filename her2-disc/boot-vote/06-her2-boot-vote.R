...
#result directory
respath <- "/home/cconley/scratch-data/neta-metabric/disc-cv-vote/her2/06/"
...
tictoc <- system.time({ens <- spacemap::bootEnsemble(Y = Y, X = X, tune = tune,
						     ...
						     resPath = respath,
                                                     seed = seed, p0 = 1.0,
                                                     tol = 1e-4, cdmax = 90e7)})
save.image(file = file.path(respath, "her2-06.rda"))
...
saveRDS(object = bv, file = file.path(respath, "her2-06-boot-vote.rds"))
save.image(file = file.path(respath, "her2-06.rda"))
